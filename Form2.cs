...
namespace WindowsFormsApplication5
{
   ...
   */
    ...
    {
        int width = 200;            //測試資料控制項之間的寬度
        int height = 150;           //測試資料欄位控制項之間的高度
        int MaxOneTestNumber = 2;   //一個測試節的最大switch連接數
        int MAX = 200;              //最大的測試數目
        int MAXcol = 10;            //最大的欄位數
        int Testnumber;             //測試節的數量
        int chapter;                //目前的章節
        string[,] pin_name;         //紀錄量測點A和B
        string[] ann;               //紀錄每個測試的測試內容
        int[,,] result;             //結果
        int[,,] exceldata;          //excel傳入結果
        
        private ComboBox[] TestType;             //測試名稱
        private ComboBox[] SwitchUseNumber;      //總共要連接的數量
        private ComboBox[,] SwitchList;          //Switch 的連接       
        private TextBox[,] SwitchRow;            //給矩陣轉換器用的row column控制
        private TextBox[,] SwitchCol;
        private TextBox[,] Channel;              //給一般channel用的控制
        private ComboBox[] ResistRange;          //給測量阻值範圍用的控制項
        private Label[] lab;                     //每個測試的編號顯示
        private Label[] title;                   //每個欄位的顯示內容
        //constructor
        public Form2(int n, int c, String[,] ss, String[,] s)
        {
            InitializeComponent();
            init(n, c); //給予控制項的宣告和初始化，n表示測試數量,c表示第幾章
            //給予註解
            ...
            for (int i = 1; i <= Testnumber; i++)//從excel讀值
            {
                //excel測試預設為阻抗量測
                exceldata[i, 1, 0] = 1;
                if (SwitchStringToint(ss[i, 3]) > 0)//當第2個測試物的switch(EXCEL上欄位為H)有符合條件的Switch時
                {
                    exceldata[i, 3, 0] = 2;         //2個switch
                    exceldata[i, 4, 0] = SwitchStringToint(ss[i, 0]);//第1個測試物的switch 在EXCEL上欄位為E
                    exceldata[i, 4, 1] = SwitchStringToint(ss[i, 3]);//第2個測試物的switch 在EXCEL上欄位為H
                    exceldata[i, 5, 0] = StringToInt(ss[i, 1]);      //第1個測試物的channel/cloumn 在EXCEL上欄位為F
                    exceldata[i, 5, 1] = StringToInt(ss[i, 4]);      //第2個測試物的channel/column 在EXCEL上欄位為I
                    exceldata[i, 6, 0] = StringToInt(ss[i, 2]);      //第1個測試物的channel/row 在EXCEL上欄位為G
                    exceldata[i, 6, 1] = StringToInt(ss[i, 5]);      //第2個測試物的channel/row 在EXCEL上欄位為J
                }
                else
                {
                    exceldata[i, 3, 0] = 1;//1個switch
                    exceldata[i, 4, 0] = SwitchStringToint(ss[i, 0]);//第1個測試物的switch 在EXCEL上欄位為E
                    exceldata[i, 5, 0] = StringToInt(ss[i, 1]);      //第1個測試物的channel/cloumn 在EXCEL上欄位為F
                    exceldata[i, 6, 0] = StringToInt(ss[i, 2]);      //第1個測試物的channel/row 在EXCEL上欄位為G
                }
        ...
        public void ToolDesign()
        {
            ...
            for (int i = 0; i <= Testnumber; i++)
            {
                ...
                if (i != 0)
                {
                    ...
                    int oneLineTextNumber = 16;    //一行的最大數字
                }
                else
                ...
                this.TestType[i].SelectedIndexChanged += new EventHandler(this.TestType_SelectedIndexChanged);    //當選擇項目改變時呼叫的function
                /*----------------------------------------*/
                //第二欄 輸入範圍
        ...
        private void start_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i <= Testnumber; i++)
            {
                ...
                if (exceldata[i, 3, 0] > 1)//開啟兩個relay
                {
