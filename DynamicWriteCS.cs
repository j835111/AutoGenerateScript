    ...
    class DynamicWriteCS
    {
        ...
        public void Dynamic_WriteTest(int test)
        {
            ...
            for (int i = 0; each_InputNum[test, i] != null; i++)//0 1 2
            {
                ...
                //MessageBox.Show(inputData[inputNumber].GetDelaySec());
                if(!string.IsNullOrEmpty(inputData[inputNumber].GetRelay[0]))
                {
                    Delay(inputData[inputNumber].GetDelaySec());
                }
            }
            WriteLine(" //*************** Measure ***********************************");
            WriteLine("");
        ...
        public void Delay(string sec)
        {
            WriteLine("//延遲 秒數:" + sec);
            WriteLine("Switch.Delay(" + sec + ")" + ";");
        }
    }
}
