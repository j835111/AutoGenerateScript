    ...
    public partial class Form3 : Form
    {
        ...
        private void Chap_choise_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                string s = chap_choise.SelectedItem.ToString();
                string sub;
                char[] chars = s.ToCharArray();
                int i = s.Length;
                for (int n = 0; n < i; n++)
                {
                    if (57 >= Convert.ToInt32(chars[n]) && Convert.ToInt32(chars[n]) >= 48)
                    {
                        sub = s.Substring(n);
                        ChapterNunber = Int32.Parse(sub);
                        break;
                    }
                }
            }
            ...
            try
            {
                ...
                for(int i=1,j=-1,k=0; !string.IsNullOrEmpty(dt1.Rows[i][3].ToString()); i++,k++)
                {
                    if(!string.IsNullOrEmpty(dt1.Rows[i][1].ToString()))
                    {
                        ...
                        inputData[j].SetData(dt1.Rows[i][2].ToString(), dt1.Rows[i][3].ToString(), dt1.Rows[i][9].ToString(), dt1.Rows[i][4].ToString(), dt1.Rows[i][10].ToString());
                        k = 0;
                    }
